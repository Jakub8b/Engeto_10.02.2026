...
WITH enter AS (
-- Táto časť načítava surové dáta o cenách potravín z pôvodných tabuliek a dopĺňa k nim názov kategórie. 
-- Zároveň extrahuje rok z dátumu, aby bolo možné vykonávať ročné agregácie.
SELECT DISTINCT
product_name,
year,
avg_price_year AS avg_price
FROM jakub_fecik_project_sql_primary_final i
),
lag_data AS (
--Táto časť dopĺňa ku každému roku cenu z predchádzajúceho roka pomocou analytickej funkcie LAG. 
-- Umožňuje tak vypočítať medziročné zmeny cien.
SELECT
product_name,
year,
ROUND(avg_price::numeric, 2) AS avg_price,
ROUND(LAG(avg_price) OVER (PARTITION BY product_name ORDER BY year)::numeric,2) AS prev_year
FROM enter
),percent_data AS (
...
SELECT*,
ROUND((avg_price - prev_year)::numeric,2) AS prev_year_diff,
ROUND((((avg_price - prev_year)/ NULLIF(prev_year, 0)) * 100)::numeric,2) AS pcnt_diff
FROM lag_data
),percent_growth as (
